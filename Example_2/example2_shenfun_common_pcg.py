...
class Example2Discretization:
    ...
    def _build_velocity_basis(self):
        ...
        self.M_sparse = (
            D_scale @ M_shen @ D_scale
        ).tocsr()
        self.S_sparse = (
            D_scale @ S_shen @ D_scale
        ).tocsr()
    ...
    def _build_matrices(self):
        # Equation (4.22): S E = M E Lambda and E.T M E = I.
        # Only this one-dimensional eigendecomposition is stored for the
        # matrix-free velocity preconditioner.
        self.velocity_eigenvalues, self.velocity_eigenvectors = eigh(
            self.S,
            self.M,
            check_finite=False,
        )
        if np.any(self.velocity_eigenvalues <= 0.0):
            raise RuntimeError(
                "The one-dimensional velocity eigenproblem is not "
                "positive definite."
            )

        self.velocity_eigenvalue_sums = (
            self.velocity_eigenvalues[:, None]
            + self.velocity_eigenvalues[None, :]
        )
    ...
    def apply_velocity_mass(self, coefficients):
        """Apply M^div by the matrix-free identity (4.32)."""
        coefficient_matrix = np.asarray(coefficients).reshape(
            self.ndof,
            self.ndof,
        )

        # Right multiplication is written as a transposed left sparse
        # multiplication so that the fixed bandwidth of M and S is used.
        s_v_m = (
            self.M_sparse
    ...
    def apply_velocity_stiffness(self, coefficients):
        """Apply S^div by the matrix-free identity (4.32)."""
        coefficient_matrix = np.asarray(coefficients).reshape(
            self.ndof,
            self.ndof,
        )
    ...
    def apply_velocity_operator(self, coefficients, gamma):
        """Apply M^div + gamma*S^div without assembling it."""
        return (
    ...
    def apply_velocity_preconditioner_inverse(self, residual, gamma):
        """Apply (M^div + gamma*tilde(S)^div)^(-1), (4.35)-(4.37)."""
        ...
        transformed_residual = (
            eigenvectors.T @ residual_matrix @ eigenvectors
        )
        eigenvalue_sums = self.velocity_eigenvalue_sums
        denominator = (
            eigenvalue_sums
            + float(gamma)*eigenvalue_sums**2
        )
        transformed_solution = transformed_residual/denominator
        solution_matrix = (
            eigenvectors
    ...
    def solve_velocity(
        ...
        """Solve the velocity system by Algorithm 1's matrix-free PCG."""
        ...
        preconditioned_residual = (
            self.apply_velocity_preconditioner_inverse(
                residual,
                gamma,
            )
        )
        ...
        if residual_inner_product <= 0.0:
            raise RuntimeError(
                "The PCG preconditioner is not positive definite."
            )
        ...
        for iteration in range(1, self.pcg_max_iterations + 1):
            ...
            denominator = float(
                search_direction @ operator_direction
            )
            if denominator <= 0.0:
                raise RuntimeError(
                    "The matrix-free velocity operator is not positive "
                    "definite."
                )
            ...
            new_residual_inner_product = float(
                residual @ new_preconditioned_residual
            )
            if new_residual_inner_product <= 0.0:
                raise RuntimeError(
                    "PCG lost positive definiteness during iteration."
                )

            direction_coefficient = (
                new_residual_inner_product/residual_inner_product
            )
            search_direction = (
                new_preconditioned_residual
                + direction_coefficient*search_direction
            )
            preconditioned_residual = new_preconditioned_residual
            residual_inner_product = new_residual_inner_product
    ...
    def gradient_norm_squared(self, coefficients):
        return float(
            coefficients @ self.apply_velocity_stiffness(coefficients)
        )
...
def save_accuracy_data(
    ...
    divergence,
    alpha_maximum,
    pcg_iterations_average=None,
    pcg_iterations_maximum=None,
):
    ...
    if (
        pcg_iterations_average is not None
        and pcg_iterations_maximum is not None
    ):
        saved_data["pcg_iterations_average"] = np.asarray(
            pcg_iterations_average,
            dtype=float,
        )
        saved_data["pcg_iterations_maximum"] = np.asarray(
            pcg_iterations_maximum,
            dtype=int,
        )
...
def print_accuracy_table(output_file, method_name):
    ...
    has_pcg_iterations = (
        "pcg_iterations_average" in data.files
        and "pcg_iterations_maximum" in data.files
    )

    print("\n" + method_name)
    header = f"{'M':>4} {'tau_max':>8}"
    if has_pcg_iterations:
        header += f" {'PCG(a/m)':>9}"
    header += (
        f" {'e_u':>8} {'ord':>4}"
        f" {'e_p':>8} {'ord':>4}"
    ...
    for index in range(len(data["number_of_steps"])):
        ...
        row = (
            f"{data['number_of_steps'][index]:4d} "
            f"{data['step_sizes'][index]:8.2e}"
        )
        if has_pcg_iterations:
            pcg_text = (
                f"{data['pcg_iterations_average'][index]:.2f}/"
                f"{data['pcg_iterations_maximum'][index]:d}"
            )
            row += f" {pcg_text:>9}"
        row += (
            f" {data['velocity_l2'][index]:8.2e} "
            f"{rate_text('velocity_l2_rate'):>4} "
            f"{data['pressure_l2'][index]:8.2e} "
            f"{rate_text('pressure_l2_rate'):>4} "
            f"{data['divergence_error'][index]:8.2e} "
            f"{rate_text('divergence_rate'):>4} "
            f"{data['alpha_error'][index]:8.2e} "
            f"{rate_text('alpha_rate'):>4}"
        )
        print(row)
